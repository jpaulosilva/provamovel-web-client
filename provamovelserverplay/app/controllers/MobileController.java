...
public class MobileController extends Controller{
	public static Result login() {
		...
		String email = json.findValue("login").asText();
		String password = json.findValue("password").asText();
		
		User busca = User.find.byId(email);
		if(busca == null){
			return ok(Json.toJson("{'error':'1','errorMessage':'Usuário não encontrado'}"));
		}else{
			if(!busca.getPassword().equals(password)){
				return ok(Json.toJson("{'error':'1','errorMessage':'Senha inválida'}"));
			}else{
